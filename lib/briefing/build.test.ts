...
import { isNeedsYouNow, type AttentionItem } from "./types"
...
describe("computeSince", () => {
  ...
  it("falls back to 24h for a first-ever visit", () => {
    expect(computeSince(null, NOW)).toBe(new Date(NOW - DEFAULT_LOOKBACK_MS).toISOString())
  })

  it("clamps a long absence to the 24h window — a briefing, not an archive", () => {
    const twoWeeks = new Date(NOW - 14 * 24 * 3_600_000).toISOString()
    expect(computeSince(twoWeeks, NOW)).toBe(new Date(NOW - DEFAULT_LOOKBACK_MS).toISOString())
  })
