...
**The lock is enforced server-side too** (2026-08-15): `app/api/draft/send/route.ts` looks up the sender's pending suggestion for the conversation and returns 409 when its band is `needs_check` unless the request carries `needsCheckConfirmed: true` — a flag the clients only set after their own two-step confirm (QueueRow's `confirming` state in `components/canvas/queue-panel.tsx`, `needsCheckConfirming` in `components/canvas/use-reply-composer.ts`). Before this, the lock was client-only and the Queue's "On request" bulk send skipped it entirely; that bulk path now also filters locked rows out and reports how many were skipped. Side effect worth knowing: a macro send (`components/canvas/macros-node.tsx`, `sendMacroAndClose` in `components/canvas/inbox-panel.tsx`) against a conversation with a pending `needs_check` draft now gets the 409 and its toast points the agent to the Queue — enforced, not silently bypassed, but with no in-flow confirm affordance there yet. The canvas reply composer DOES recover in-flow (2026-08-15): its `riskBand` is a one-shot snapshot, so when a draft is superseded into `needs_check` behind its back the 409 flips the composer into its normal confirm-to-send step (`forcedNeedsCheck` in `use-reply-composer.ts`) without losing the agent's text — the next click is the explicit confirm.
