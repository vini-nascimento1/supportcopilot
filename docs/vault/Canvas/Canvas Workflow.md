...
- `lib/canvas-hotkeys.ts` — keyboard shortcuts shared across all three sidebar tabs: Ctrl/Cmd+A toggles select-all, Ctrl/Cmd+Enter fires the active tab's primary bulk action (Inbox: Generate/Assign, Queue: Approve & send, Triage: Assign+draft). Suppressed while the focus is inside a text input. Select-all semantics are identical on all three tabs (a partial selection EXTENDS to all; only a full selection clears — Triage was aligned 2026-08-15), and Ctrl/Cmd+Enter never executes an outward-facing bulk action directly: Inbox and Queue both arm a visible confirm first, second press runs it.
...
- **Anchored cards re-clamp on real resizes** (2026-08-15): `ToolNode` tracks the anchor layer's size with a `ResizeObserver` in state (ignoring 0×0 readings from hidden keep-alive panes) instead of reading `clientWidth/Height` during render — so shrinking the window re-clamps a pinned card into the visible pane instead of stranding it off-screen. Keep-alive tab switches also reopen a tool at the URL the agent last navigated to (`liveUrlRef`), not the card's original `data.url`, and reset find-in-page state the fresh view can't self-correct.
- **Layout saves are PII-minimised** (2026-08-15): the debounced save runs node data through `dataForSave()` — conversation nodes persist only `{playbookId, playbookName, copilotTranscript}`, case-info only `{overrides}`, macros only `{conversationId}`; the full Intercom `messages` thread and customer contact details are never written to `localStorage` (loadLayout always re-seeds them from fresh props). A stored `{nodes: []}` layout is respected as an intentionally cleared canvas (case canvases still re-inject the essential Case Info/Conversation cards; ad-hoc canvases stay truly empty). On the web build (no desktop host) the save/poll/playbook effects are skipped entirely — the canvas is behind the desktop download gate there. The `canvas-add-tool` palette listener is gated on the active pane, and the command palette's "Open on canvas" recognises `/workspace` as a canvas surface.
...
The lock rule is identical everywhere: a `needs_check` draft is marked **Locked**, renders the lock reason (`LOCK_REASON_GENERIC` from `lib/briefing/format.ts`, the same copy [[Home Briefing]] uses), and only sends through the locked variant of `SendConfirmDialog` (`locked` prop), whose confirm button asserts that the agent has done the fadmin check; that confirm is what sets `needsCheckConfirmed: true`. Sending works from a phone; only fadmin and the Canvas need the desktop app, so the row also offers "Check on desktop" / "Check in fadmin" and "Open in Intercom". `components/home/prepared-card.tsx` follows the same rule. The server enforces it independently — `/api/draft/send` answers 409 for a pending `needs_check` suggestion without the confirmation. Unassigned rows offer "Assign to me" first, exactly as in Canvas, and every send goes through the shared `SendConfirmDialog` so nothing leaves without an explicit tap plus a confirmation.
