...
1. A per-agent OAuth user token, `agents.slack_token`, connected from the Settings page (see [[Settings and Profile]]) via `/api/auth/slack` → Slack OAuth consent → `/api/auth/slack/callback`. The `redirect_uri` is built from the request origin, so every domain the app is served from (`https://fanvuecopilot.vercel.app` in production) must be listed under *OAuth & Permissions → Redirect URLs* on the Support Copilot Slack app (`A0B8DR470DV`); otherwise Slack answers "redirect_uri did not match any configured URIs" and the connect fails before the callback runs. A successful connect (and a disconnect from Settings) calls `invalidateBriefingCache` so [[Home Briefing]] rebuilds instead of showing the stale "Connect Slack" state for up to five minutes.
